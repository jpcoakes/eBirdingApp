...
export function handleOptionClickAction(event, compObj) {
  ...
  if (!compObj.comboboxObj.multiselect) {
    // if multiselect is not enabled then Don't close the Popup
    openAndCloseDropdown("close", compObj);
  }
}
...
export function handleOptionHoverAction(event, compObj) {
  ...
  if (event.type === "mouseover") {
    const currentHoverIndex = getOptionElementIndexById(
...
export function handleSelectionRemovalAction(event, compObj) {
  ...
  if (value && compObj.comboboxObj._selectedValues.length > 0) {
    value = compObj.comboboxObj._selectedValues.find(
...
export function evaluateExpression(value, condition) {
  return new Function("value", '"use strict";return (' + condition + ")")(
    value
  );
}
...
function populateValueOnInput(value, comboboxObj) {
  ...
  if (value && typeof value === "string") {
    value = comboboxObj.options.find(
      (opt) => opt.value.toString() === value.toString()
    );
  }
...
function filterDropdownList(event, compObj) {
  ...
  if (compObj.comboboxObj._searchTerm != searchTerm) {
    ...
    if (compObj.comboboxObj.lazySearch && searchTerm.length >= 3) {
      ...
      compObj.comboboxObj.options = compObj.comboboxObj.dropdownList.filter(
        (ele) => ele.label.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  }
}
...
function findCurrentHoverIndex(template) {
  ...
  const listElements = template.querySelectorAll('div[role="option"]');
  if (listElements.length > 0) {
    return [...listElements].findIndex(
      (ele) => ele.getAttribute("aria-selected") === "true"
    );
  }
  return -1;
}
...
function findCurrentSelectedIndex(template) {
  ...
  if (listElements.length > 0) {
    return [...listElements].findIndex(
      (ele) => ele.getAttribute("aria-checked") === "true"
    );
  }
  return -1;
}
...
function handleEnterKey(event, compObj) {
  ...
  if (!compObj.comboboxObj._isDropdownVisibile) {
    ...
    if (currentIndex >= 0) {
      ...
      let value =
        template.querySelectorAll('div[role="option"]')[currentIndex].dataset
          .value;
      populateValueOnInput(value, compObj.comboboxObj);
      if (!compObj.comboboxObj.multiselect) {
        openAndCloseDropdown("close", compObj);
...
function getOptionElementIndexById(elementId, template) {
  ...
  if (listElements.length > 0) {
    ...
    return [...listElements].findIndex(
      (ele) => ele.getAttribute("id") === elementId
    );
  }
  return -1;
}
...
function highlightSelection(index, template) {
  ...
  if (listElements.length > 0) {
    ...
    //selection-input
    template
      .querySelector("input.selection-input")
