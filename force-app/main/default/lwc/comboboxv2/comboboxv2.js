  ...
  VALIDATION = [
    {
      name: "REQUIRED",
      condition:
        "(value === null || value === undefined || (typeof value === 'string' && value.trim() === '')) ? true : false",
...
export default class Combobox extends LightningElement {
  ...
  @track comboboxObj = {
    ...
    get _defaultSearchText() {
      return this.isLoading
        ? LOADING_TEXT
        : this._searchTerm && this._searchTerm.length >= 3
          ? NO_MATCH_FOUND + '"' + this._searchTerm + '"'
          : DEFAULT_SEARCH_TEXT;
    },
    get autocomplete() {
    ...
    set value(val) {
      ...
      if (typeof val === "object") {
        ...
        if (typeof val === "string" && val.trim() === "") {
          ...
          try {
            ...
            console.warn(
              "Supplied value is not a valid Object. Converting to JS Object"
            );
        ...
        if (this._selectedValues.length > 0) {
          if (this.multiselect && !Array.isArray(val)) {
            ...
            const valueIndex = valueList.findIndex(
              (ele) => ele.value.toString() === val.value.toString()
            );
            if (valueIndex > -1) {
      ...
      if (!compareArrayObject(valueList, this._selectedValues)) {
    ...
    get _value() {
      return this._selectedValues
        ? this._selectedValues.map((obj) => obj.value).join(SEPERATOR)
        : "";
    },
    get _clearMode() {
      return this.autocomplete &&
    ...
    get _computedInputClass() {
      return this._clearMode
        ? "slds-input slds-combobox__input selection-input slds-combobox__input-value"
        : "slds-input slds-combobox__input selection-input";
    },
    get _dropdownTriggerClass() {
      return this._isDropdownVisibile
    ...
    get _computedFormElementClass() {
      return this.hasError
        ? this._computedFormLayoutVariant + " slds-has-error"
        : this._computedFormLayoutVariant;
    },
  ...
  set options(value) {
    if (typeof value === "object" && value.length > 0) {
      this.comboboxObj.options = this.prepareDropdownOptionList(
        deepClone(value)
      );
      this.comboboxObj.dropdownList = this.prepareDropdownOptionList(
        deepClone(value)
      );
    } else {
      ...
      console.warn(
        'An Empty object passed in Combobox[options] attribute, please check the passed "options" parameter'
      );
    }
  }
  ...
  connectedCallback() {
    ...
    if (!this.comboboxObj.lazySearch) {
      // if it is not a lazysearch
  ...
  handleInputKeyDown(event) {
    ...
    if (
      this.comboboxObj.readOnly ||
      this.comboboxObj.disabled ||
      (this.comboboxObj._disableDropdown &&
        ...
        get() {
          ...
          if (this._label) {
            ...
            if (!comboboxObj.lazySearch) {
              const obj1 = comboboxObj.dropdownList.find(
                (opt) => opt.value.toString() === this.value.toString()
              );
              if (obj1) {
                label = obj1.label;
              } else {
  ...
  trimOptionList() {
    ...
    if (selectedValues != null && selectedValues.length > 0) {
      const partialOptions = allOptions.filter((optionItem) =>
        selectedValues.some(
          (seledtedOption) => seledtedOption.value === optionItem.value
        )
      );
  ...
  setCustomErrorMessage(msg, cond) {
    ...
    const objIndex = this.comboboxObj.validation.findIndex(
      (obj) => obj.name === dataObj.name
    );
    if (objIndex > -1) {
  ...
  runValidationRules() {
    ...
    if (errorObj && errorObj.length > 0) {
      const filteredObj = errorObj.filter((error) => {
        return (
          this.comboboxObj._builtInValidation.includes(error.name) ||
          error.custom
        );
      });
