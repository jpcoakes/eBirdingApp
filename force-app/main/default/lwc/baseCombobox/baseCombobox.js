...
export default class cBaseCombobox extends LightningElement {
  ...
  handleInput(event) {
    ...
    // sort the items list based on the inputElement value
  ...
  filterItems(string) {
    this._items = this.items
      .filter((option) =>
        option.text.toLowerCase().includes(string.toLowerCase())
      )
      .map((item) => {
        let index = item.text.toLowerCase().indexOf(string.toLowerCase());
        let newObj = { ...item };
        if (index === 0) {
          newObj.text = [
            {
              text: newObj.text.substring(0, string.length),
              highlight: true
            },
            {
              text: newObj.text.substring(string.length)
            }
          ];
        } else {
          newObj.text = [
            { text: newObj.text.substring(0, index) },
            {
              text: newObj.text.substring(index, index + string.length),
              highlight: true
            },
            { text: newObj.text.substring(index + string.length) }
          ];
        }
        return newObj;
      });
    this._items = this.sortItems(this._items, string);
